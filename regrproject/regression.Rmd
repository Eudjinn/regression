...
```
From the coefficients we can see that for manual transmission (factor(am)1), the estimated change of *mpg* is `r summary(fit3)$coef[2,1]` compared to automatic transmission with *hp* and *wt* fixed. However, **looking at p-value = `r summary(fit3)$coef[2,4]` which is bigger than 0.05, the change in *mpg* is not statistically significant**. Based on this result, the answers for the questions are provided below:  
